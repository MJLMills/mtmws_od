MAXIMUM_TIMER_FREQUENCY_HZ = 250
...
loop, which is set by the model update and output socket timings.
...
This value has no mathematical lower limit, must be arbitrarily chosen.
"""
...
"""
Determines how much slower Lorenz system B is than Lorenz system A.
...
extrema_range = 0.0  # does not work if this is non-zero, why?

timestep_a = RangedVariable(
    minimum=RangedVariable(minimum=MINIMUM_TIMESTEP-extrema_range,
                           maximum=MINIMUM_TIMESTEP+extrema_range,
                           value=MINIMUM_TIMESTEP),
    maximum=RangedVariable(minimum=MAXIMUM_TIMESTEP-extrema_range,
                           maximum=MAXIMUM_TIMESTEP+extrema_range,
...
timestep_b = RangedVariable(
    minimum=RangedVariable(minimum=b_minimum_timestep-extrema_range,
                           maximum=b_minimum_timestep+extrema_range,
                           value=b_minimum_timestep),
    maximum=RangedVariable(minimum=b_maximum_timestep-extrema_range,
                           maximum=b_maximum_timestep+extrema_range,
...
cv_magnitude_a_input_socket.jack_removed.connect(
    ...
    lambda: cv_magnitude_knob.value_changed.connect(
...
timestep_cv_input_socket.jack_inserted.connect(
    ...
    # connect the timestep knob to the min/max of the timesteps of A and B
    lambda: timestep_knob.value_changed.connect(lorenz_attractor_a.timestep.minimum.map_value),
    lambda: timestep_knob.value_changed.connect(lorenz_attractor_a.timestep.maximum.map_value),
    lambda: timestep_knob.value_changed.connect(lorenz_attractor_b.timestep.minimum.map_value),
    lambda: timestep_knob.value_changed.connect(lorenz_attractor_b.timestep.maximum.map_value),
...
timestep_cv_input_socket.jack_removed.connect(
    ...
    lambda: timestep_knob.value_changed.disconnect(lorenz_attractor_a.timestep.maximum.map_value),
    lambda: timestep_knob.value_changed.disconnect(lorenz_attractor_b.timestep.minimum.map_value),
    lambda: timestep_knob.value_changed.disconnect(lorenz_attractor_b.timestep.maximum.map_value),
